...
width = scalling * 32
height = scalling * 24
# backSub = cv2.createBackgroundSubtractorMOG2(history=500,varThreshold = 32)
...
mlx.refresh_rate = adafruit_mlx90640.RefreshRate.REFRESH_16_HZ # 16Hz is noisy
time.sleep(1)
...
while True:
        if index < 10:
            ...
            ret,fgmask2 = cv2.threshold(new_imgGray,180,255,cv2.THRESH_BINARY)
            ...
            if (current_region[1] != prev_region[1]) and (current_region[0] > prev_region[0]):
                totalNofPeople = totalNofPeople + 1

            elif (current_region[1] != prev_region[1]) and (current_region[0] < prev_region[0]):
                if totalNofPeople > 0:
                    totalNofPeople = totalNofPeople - 1
            ...
            cv2.imshow("Bsubtraction",fgmask2)
